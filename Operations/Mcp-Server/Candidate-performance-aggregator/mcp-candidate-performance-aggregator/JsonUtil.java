...
public final class JsonUtil {
    ...
    // Public parser for parsing JSON arrays/objects externally
    ...
    static class Parser {
        ...
        Map<String,Object> parseObj() {
            ...
            while(true){skipWs();String k=parseStr();skipWs();expect(':');skipWs();
                m.put(k,parse());skipWs();if(peek()=='}'){pos++;break;}expect(',');}
            return m;
        }
        ...
        Object parseLit(String w,Object v){if(src.startsWith(w,pos)){pos+=w.length();return v;}throw new IllegalArgumentException("Bad literal");}
        Object parseNum(){
        ...
        void expect(char c){if(pos>=src.length()||src.charAt(pos)!=c)throw new IllegalArgumentException("Expected '"+c+"' at "+pos);pos++;}
    }
}
